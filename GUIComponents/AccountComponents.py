...
class UserAccountGUI:
    ...
    def openAccount(self):
        try:
            ...
            logger.error(f"Error while opening account. {error}")
            ...
            if selectedCustomer.getAccount() is None:
                if selectedCustomer.getType() == "Retail":
                    ...
                    customtkinter.CTkButton(text_font=("Malgun Gothic", 10), image=self.confirm, master=frame,
                                            ...
                                            compound="top",
                                            command=lambda: [
                                                self._AccountService.addAccount(selectedCustomerName, amount.get(),
                                                                                var.get()),
                                                frame.destroy(),
                                                AccountOpened.createConfirmationPopUp("Account Opened.")]).place(relx=0.2,
                                                                                                                 rely=0.65,
                                                                                                                 anchor='center')
                    ...
                    customtkinter.CTkButton(text_font=("Malgun Gothic", 10), image=self.confirm, master=frame,
                                            ...
                                            compound="top",
                                            command=lambda: [
                                                self._AccountService.addAccount(selectedCustomer, amount, "Loan"),
                                                frame.destroy()]).place(relx=0.2, rely=0.65, anchor='center')

            else:
                errorExists=ErrorPopUp('Customer has an account.')
                errorExists.createErrorPopUp()
        except Exception as ex:
            logger.error(f"Problem while making account.{ex}")

    def listAccounts(self):
        try:
            frame = customtkinter.CTkFrame(self._rootWindow, width=1000, height=800)
            frame.grid(row=0, column=0, sticky='news')
            logger.info("Listing money in accounts.")
            ...
            logger.error(f"Problem while displaying money in accounts. {e}")
