        ...
        try:
            for employee in EmployeeList:
                ...
                if employee['type'] == "createSoftwareEngineer":
                    newEmp = SoftwareEngineer(employee['name'], employee['age'], employee['joiningDate'],
                                              employee['type'])
                elif employee['type'] == "createITStaff":
                    newEmp = ITstaff(employee['name'], employee['age'], employee['joiningDate'], employee['type'])
                elif employee['type'] == "createMaid":
                    newEmp = Maid(employee['name'], employee['age'], employee['joiningDate'], employee['type'])
                elif employee['type'] == "createArchitect":
                    newEmp = Architect(employee['name'], employee['age'], employee['joiningDate'], employee['type'])
                if employee['manager'] is not None:
                    newEmp = Manager(employee['name'], employee['age'], employee['joiningDate'], employee['type'])
        ...
        try:
            for customer in CustomerList:
                if customer["accountDetails"] is not None:
                    ...
                    newAccount.setAccountOpeningTime(datetime.datetime.strptime(customer["accountDetails"]["timeOfAccountOpening"], "%d/%m/%y %H:%M:%S"))
